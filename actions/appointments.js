...
const VONAGE_APPLICATION_ID_REGEX =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

let cachedVonageClient = null;

function getVonageConfig() {
  const applicationId =
    process.env.VONAGE_APPLICATION_ID ||
    process.env.NEXT_PUBLIC_VONAGE_APPLICATION_ID;
  const privateKeyValue = process.env.VONAGE_PRIVATE_KEY;

  if (!applicationId) {
    throw new Error(
      "Vonage is not configured. Add VONAGE_APPLICATION_ID (or NEXT_PUBLIC_VONAGE_APPLICATION_ID) to your .env file."
    );
  }

  if (!VONAGE_APPLICATION_ID_REGEX.test(applicationId)) {
    throw new Error(
      "Vonage application ID is invalid. Update VONAGE_APPLICATION_ID in .env with the Application ID from your Vonage dashboard."
    );
  }

  if (!privateKeyValue) {
    throw new Error(
      "Vonage private key is missing. Set VONAGE_PRIVATE_KEY in your .env file."
    );
  }
  ...
  if (!privateKeyValue.includes("BEGIN PRIVATE KEY")) {
    const privateKeyPath = path.isAbsolute(privateKeyValue)
      ? privateKeyValue
      : path.join(process.cwd(), privateKeyValue);

    if (!fs.existsSync(privateKeyPath)) {
      throw new Error(
        `Vonage private key file was not found at ${privateKeyPath}.`
      );
    }
...
function getVonageClient() {
  if (cachedVonageClient) {
    return cachedVonageClient;
  }

  const { applicationId, privateKey } = getVonageConfig();
  const credentials = new Auth({ applicationId, privateKey });

  cachedVonageClient = new Vonage(credentials, {});
  return cachedVonageClient;
}
...
async function createVideoSession() {
  try {
    ...
    if (error?.response?.status === 401 || message.includes("status code 401")) {
      throw new Error(
        "Vonage authentication failed. Check VONAGE_APPLICATION_ID and VONAGE_PRIVATE_KEY in your .env file."
      );
    }
